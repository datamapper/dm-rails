...
Once you have bundler installed, you can bootstrap a rails3 application with a single command. Yes! A single command. And to double the fun, you can either choose to run off rails-3.0.0.beta or off rails master branch. Cool.
...
In the future, http://github.com/snusnu/rails-templates will host a more comprehensive set of rails application templates. Currently only the above are supported but you can expect some reasonable defaults to follow soon. Things like rspec and jquery support are definitely on the list, maybe also support for {devise}[http://github.com/plataformatec/devise] and {inherited_resources}[http://github.com/josevalim/inherited_resources].
